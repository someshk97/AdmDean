<?php
    ...
    $file_extension = pathinfo($_FILES["gradcert_input"]["name"], PATHINFO_EXTENSION); 
    //echo $file_extension;
    ...
    else if ($_FILES["gradcert_input"]["size"] > 500000)   // Validate image file size <500KB
    {   
        $response = array(
            "type" => "error",
            "message" => "PDF size exceeds 500KB."
        );
        echo $_FILES["gradcert_input"]["size"];
    }
    else if ($_FILES["gradcert_input"]["size"] ===0)   // Validate image file size zero means huge file
    {   
        $response = array(
            "type" => "error",
		...
		$target = $application_folder."/" . $application_id . "_GRADCERT." .$file_extension;
		//echo $target;
        //$target = "image/" . basename($_FILES["gradcert_input"]["name"]);
        if (move_uploaded_file($_FILES["gradcert_input"]["tmp_name"], $target)) {//Uploaded successfully so insert photo upload status into docs upload status table
			$con = mysqli_connect(ORA_CON_DBHOSTNAME, ORA_CON_USER, ORA_CON_PASSWORD, ORA_CON_DATABASE);
			$sql="update t_docs_upload_status SET gradcert_upload_status = 'SUCCESSFUL' WHERE s_detid = '".$application_id."'";
