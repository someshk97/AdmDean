<?php
	...
	$response = array();
    // Get Image Dimension
    $fileinfo = @getimagesize($_FILES["sign_input"]["tmp_name"]);
    $width = $fileinfo[0];
    $height = $fileinfo[1];
    ...
    $file_extension = pathinfo($_FILES["sign_input"]["name"], PATHINFO_EXTENSION);
    
    // Validate file input to check if is not empty
    if (! file_exists($_FILES["sign_input"]["tmp_name"])) {
        $response = array(
            "type" => "error",
    ...
    else if (($_FILES["sign_input"]["size"] > 200000)) {
        $response = array(
            "type" => "error",
        ...
        $response = array(
            "type" => "error",
            "message" => "Image dimension should be within 300px X 200px"
        );
    } else {
		$target = $application_folder."/" . $application_id . "_SIGN." .$file_extension;
		//echo $target;
        //$target = "image/" . basename($_FILES["sign_input"]["name"]);
        if (move_uploaded_file($_FILES["sign_input"]["tmp_name"], $target)) {//Uploaded successfully so insert photo upload status into docs upload status table
			$con = mysqli_connect(ORA_CON_DBHOSTNAME, ORA_CON_USER, ORA_CON_PASSWORD, ORA_CON_DATABASE);
			$sql="update t_docs_upload_status SET sign_upload_status = 'SUCCESSFUL' WHERE s_detid = '".$application_id."'";
