<?php
    ...
    $file_extension = pathinfo($_FILES["icard_input"]["name"], PATHINFO_EXTENSION);	
    ...
    else if ($_FILES["icard_input"]["size"] > 500000) 	// Validate image file size <500KB
	{	
        $response = array(
            "type" => "error",
            "message" => "PDF size exceeds 500KB."
        );
		echo $_FILES["icard_input"]["size"];
    }
	else if ($_FILES["icard_input"]["size"] ===0) 	// Validate image file size zero means huge file
	{	
        $response = array(
            "type" => "error",
	...
	{
		$target = $application_folder."/" . $application_id . "_ICARD." .$file_extension;
		
		if (move_uploaded_file($_FILES["icard_input"]["tmp_name"], $target)) 
		{	//Uploaded successfully so insert photo upload status into docs upload status table
			...
			$sql="update t_docs_upload_status SET icard_upload_status = 'SUCCESSFUL' WHERE s_detid = '".$application_id."'";
			if (mysqli_query($con, $sql)) 
			{
			  $response = array(
                "type" => "success",
                "message" => "ICARD uploaded successfully and Document Upload Status updated successfully!"
             );			  	  
			} 
			else 
			{
			  $response = array(
                "type" => "success",
                "message" => "ICARD uploaded successfully But Document Upload Status Could Not be updated!"
             );
