<?php
    ...
    $file_extension = pathinfo($_FILES["exp_input"]["name"], PATHINFO_EXTENSION); 
    //echo $file_extension;
    ...
    else if ($_FILES["exp_input"]["size"] > 500000)   // Validate image file size <500KB
    {   
        $response = array(
            "type" => "error",
            "message" => "PDF size exceeds 500KB."
        );
        echo $_FILES["exp_input"]["size"];
    }
    else if ($_FILES["exp_input"]["size"] ===0)   // Validate image file size zero means huge file
    {   
        $response = array(
            "type" => "error",
		...
		$target = $application_folder."/" . $application_id . "_EXP." .$file_extension;
		//echo $target;
        //$target = "image/" . basename($_FILES["exp_input"]["name"]);
        if (move_uploaded_file($_FILES["exp_input"]["tmp_name"], $target)) {
			...
			$sql="update t_docs_upload_status SET exp_upload_status = 'SUCCESSFUL' WHERE s_detid = '".$application_id."'";
